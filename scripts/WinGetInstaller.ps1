...
Invoke-ExternalCommand -Command { 
    ...
    try {
        ...
        [void]([bool]$DebugMode = $false)
        [void]([bool]$DisableCleanup = $false)
        [void]([bool]$Force = $false)
        [void]([bool]$ForceClose = $false)
        [void]([bool]$CheckForUpdate = $false)
        [void]([bool]$UpdateSelf = $false)
        [void]([bool]$Version = $false)
        [void]([bool]$Help = $false)
        ...
        function Get-OSInfo {
            ...
            try {
                ...
                throw "Unable to get OS version details.`nError: $_"
                ExitWithDelay 1
            }
        }
        ...
        function Get-GitHubRelease {
            ...
            try {
                ...
                throw "Unable to check for updates.`nError: $_"
                exit 1
            }
        }
        ...
        function ExitWithDelay {
            ...
            if ($ExitCode -ne 0) { throw "Winget Installation Error" }
        ...
        if ($osVersion.Type -eq "Workstation" -and $osVersion.NumericVersion -lt 10) {
            throw "winget is only compatible with Windows 10 or greater."
            ExitWithDelay 1
        }
        ...
        if ($osVersion.Type -eq "Workstation" -and $osVersion.NumericVersion -eq 10 -and $osVersion.ReleaseId -lt 1809) {
            throw "winget is only compatible with Windows 10 version 1809 or greater."
            ExitWithDelay 1
        }

        # If it's a server, it needs to be 2022+
        if ($osVersion.Type -eq "Server" -and $osVersion.NumericVersion -lt 2022) {
            throw "winget is only compatible with Windows Server 2022+."
            ExitWithDelay 1
        }
