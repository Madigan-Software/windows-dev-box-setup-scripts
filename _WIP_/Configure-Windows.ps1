<#
.SYNOPSIS
Windows 10 or Server 2016 Initial Setup Script.
.DESCRIPTION
Post intall Windows configuration script. This is meant to be run manually and can be used in conjunction with a config.json file (that this script can also create). To create a boxstarter version of this script use the create-windows-config.ps1 script with this project instead.
.PARAMETER Configuration
Configuration file to load or save
.PARAMETER RequireAdmin
Changes being made will require an elevated prompt
.PARAMETER WaitForKeyPress
Wait for key press before rebooting.
.PARAMETER Reboot
Reboot after completion of setting changes
.PARAMETER SkipProcessing
Do everything but the actual processing of changes. Good for creating configuration files only
.PARAMETER ForceInteractive
Force prompting for configuration settings
.EXAMPLE
PS> .\Configure-Windows.ps1 -Whatif

Runs the script. If there is no existing settings file found the user will be prompted for settings to activate (and prompted again to save them). All actions that would have occurred if this were run will be displayed on the page but nothing will happen (including the saving of the the configuration file).

.EXAMPLE
PS> .\Configure-Windows.ps1 -SkipProcessing

Runs the script. If there is no existing settings file found the user will be prompted for settings to activate and prompted again to save them. Nothing else will happen.
.NOTES
Author: Disassembler <disassembler@dasm.cz>
ReAuthored By: Zachary Loeber
...
param(
    [Parameter(HelpMessage = 'Configuration file to load or save.')]
    [string]$Configuration = (Join-Path $PSScriptRoot 'config.json'),
    [Parameter(HelpMessage = 'Restarts this script elevated if required.')]
    [bool]$RequireAdmin = $true,
    [Parameter(HelpMessage = 'Waits for a key press before proceeding with the reboot operation.')]
    [bool]$WaitForKeyPress = $true,
    [Parameter(HelpMessage = 'Reboot the local machine at the end of processing.')]
    [bool]$Reboot = $true,
    [Parameter(HelpMessage = 'Skips performing any processing of changes. Good for simply creating a configuration file (to see what would have processed use -whatif instead)')]
    [bool]$SkipProcessing = $False,
    [Parameter(HelpMessage = 'Force interactive prompts for configuration settings')]
...
process {}
    ...
    if ( -not $SkipProcessing ) {
        if ($RequireAdmin) {
            if ($pscmdlet.ShouldProcess('Script', 'RequireAdmin')) {
                ReRunScriptElevated
            }
        }
    }
    ...
    if (Test-Path $Configuration) {
        Write-Host "Loading configuration file: $Configuration"
        try {
            $tweaks = Get-Content $Configuration | ConvertFrom-Json
        }
        catch {
            throw 'Unable to load configuration file!'
        }
    }
    if ($ForceInteractive -or ($null -eq $tweaks)) {
        Write-Host 'Starting interactive prompting for configuration settings..'
        try {
        ...
        $SaveConfig = Read-Choice -Message 'Save configuration for later?' -Choices @('&Yes', '&No') -Title 'Save choices?'

        if ($SaveConfig -eq 0) {
            try {
                $tweaks | ConvertTo-Json | Out-File -FilePath $Configuration -Encoding:utf8 -Force
                Write-Output "Configuration file has been saved to $Configuration"
            }
            catch {
    ...
    if ( -not $SkipProcessing ) {
        if ($tweaks.count -gt 0) {
            ...
            $tweaks | ForEach-Object {
                if ($pscmdlet.ShouldProcess('Local System', $_)) {
                    Invoke-Expression $_
                }
            }
            ...
            if ($Restart) {
                if ($pscmdlet.ShouldProcess('Local System', 'Restart')) {
                    Restart-Computer
                }
            }
        ...
        Write-Output 'No processing performed. Run this script with -WhatIf to see what might have been processed.'
    }
}